"""
Generalized N-Fold Training — ResNet-based Attention Fusion (Improved)
Saves    : results_resnet_attn_improved/
...
SAVE_DIR  = "results_resnet_attn_improved"
IMG_SIZE  = 96
BATCH     = 8 * len(GPU_IDS)
EPOCHS    = 50
LR        = 1e-4
...
class GliomaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Augmentation (training only) — safe left-right flip
        if self.augment:
...
class ResNetModalityEncoder(nn.Module):
    ...
    def __init__(self, feat_dim=256):
        ...
        self.layer1 = nn.Sequential(ResBlock3D(32,  32),
                                    ResBlock3D(32,  32))
        self.layer2 = nn.Sequential(ResBlock3D(32,  64,  stride=2),
                                    ResBlock3D(64,  64))
        self.layer3 = nn.Sequential(ResBlock3D(64,  128, stride=2),
                                    ResBlock3D(128, 128))
        self.layer4 = nn.Sequential(ResBlock3D(128, feat_dim, stride=2),
                                    ResBlock3D(feat_dim, feat_dim))
...
class ResNetAttentionFusionNet(nn.Module):
    ...
    def __init__(self, num_modalities, feat_dim=256, num_classes=2, dropout=0.5):
        ...
        # Expanded classifier head for better AUC
...
def train_one_fold(model_fn, X_train, y_train, X_val, y_val, modality_files):
    ...
    # CosineAnnealingLR — more stable across folds than ReduceLROnPlateau
    ...
    train_ds = GliomaDataset(
        X_train, y_train, DATA_DIR, modality_files, IMG_SIZE,
        dropout_p=DROPOUT_P,
        augment=True,                               # augmentation ON for train
        cache_dir=CACHE_DIR if USE_CACHE else None)
    val_ds = GliomaDataset(
        X_val, y_val, DATA_DIR, modality_files, IMG_SIZE,
        augment=False,                              # augmentation OFF for val
        cache_dir=CACHE_DIR if USE_CACHE else None)
    ...
    for epoch in range(EPOCHS):
        ...
        # CosineAnnealingLR steps per epoch, no argument needed
        scheduler.step()
        ...
        with torch.no_grad():
            for imgs, lbls in val_loader:
                ...
                all_preds.extend(logits.float().argmax(1).cpu().numpy())
    ...
    for sc_name, avail_ch in TEST_SCENARIOS.items():
        ...
        with torch.no_grad():
            for imgs, lbls in loader:
                ...
                preds = logits.float().argmax(1).cpu().numpy()
...
for fold in RUN_FOLDS:
    ...
    scenario_results = evaluate_scenarios(
        model, X_val, y_val, MODALITY_FILES, is_attention=True)
