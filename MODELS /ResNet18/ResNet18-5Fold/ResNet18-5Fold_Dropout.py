...
Dropout : Dropout3d in ResBlocks + stem; two-stage Dropout in classifier head
"""
...
BLOCK_DROP = 0.1      # ← Dropout3d rate inside each ResBlock and stem
HEAD_DROP  = 0.5      # ← Dropout rate in the classifier head

print(f"Batch size   : {BATCH}")
print(f"Image size   : {IMG_SIZE}³")
print(f"CV folds     : {N_FOLDS}")
print(f"Block dropout: {BLOCK_DROP}")
print(f"Head dropout : {HEAD_DROP}")
...
# ─── 3D RESNET18 WITH DROPOUT ─────────────────────────────────────────────────
class ResBlock3D(nn.Module):
    """
    Standard residual block with an added Dropout3d between the two convolutions.
    Dropout3d drops entire feature-map channels, which is more effective than
    voxel-wise dropout for 3-D convolutional features.
    """
    def __init__(self, in_ch, out_ch, stride=1, drop_rate=0.1):
        super().__init__()
        self.conv1 = nn.Conv3d(in_ch, out_ch, 3, stride=stride, padding=1, bias=False)
        self.bn1   = nn.BatchNorm3d(out_ch)
        self.relu  = nn.ReLU(inplace=True)
        self.drop  = nn.Dropout3d(p=drop_rate)   # ← channel-wise spatial dropout
    ...
    def forward(self, x):
        ...
        out = self.drop(out)                      # ← applied after first BN+ReLU
...
class ResNet18_3D(nn.Module):
    """
    3-D ResNet-18 with dropout at three locations:
      1. Stem      : Dropout3d(p=block_drop) after the initial BN+ReLU
      2. ResBlocks : Dropout3d(p=block_drop) between the two conv layers in each block
      3. Head      : two-stage Dropout(p=head_drop) in the classifier
                     (before and after a hidden Linear(512→256) layer)
    """
    def __init__(self, in_channels=4, num_classes=2,
                 block_drop=0.1, head_drop=0.5):
        super().__init__()
        ...
        self.stem = nn.Sequential(
            ...
            nn.Dropout3d(p=block_drop),           # ← dropout in stem
            nn.MaxPool3d(kernel_size=3, stride=2, padding=1)
        )

        # ── Residual layers ───────────────────────────────────────────────────
        self.layer1 = nn.Sequential(
            ResBlock3D(64,  64,  drop_rate=block_drop),
            ResBlock3D(64,  64,  drop_rate=block_drop)
        )
        self.layer2 = nn.Sequential(
            ResBlock3D(64,  128, stride=2, drop_rate=block_drop),
            ResBlock3D(128, 128, drop_rate=block_drop)
        )
        self.layer3 = nn.Sequential(
            ResBlock3D(128, 256, stride=2, drop_rate=block_drop),
            ResBlock3D(256, 256, drop_rate=block_drop)
        )
        self.layer4 = nn.Sequential(
            ResBlock3D(256, 512, stride=2, drop_rate=block_drop),
            ResBlock3D(512, 512, drop_rate=block_drop)
        )

        # ── Classifier head ───────────────────────────────────────────────────
        # Two-stage dropout with a hidden layer for stronger regularisation.
        # head_drop is applied both before and after the hidden Linear(512→256).
        self.head = nn.Sequential(
            nn.AdaptiveAvgPool3d(1),
            nn.Flatten(),
            nn.Dropout(p=head_drop),              # ← dropout before hidden layer
            nn.Linear(512, 256),
            nn.ReLU(inplace=True),
            nn.Dropout(p=head_drop),              # ← dropout after hidden layer
...
def build_model(block_drop=BLOCK_DROP, head_drop=HEAD_DROP):
    m = ResNet18_3D(block_drop=block_drop, head_drop=head_drop).to(DEVICE)
    if MULTI_GPU:
        m = nn.DataParallel(m)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(subjects, labels)):
    ...
    for epoch in range(EPOCHS):
        ...
        # --- Validation ---
        model.eval()   # disables all dropout layers during evaluation
...
def pad_nan(lst):
    """Pad shorter fold histories with NaN so we can nanmean across folds."""
...
for f_idx, aucs in enumerate(all_fold_val_aucs):
    ...
    ax.plot(ep, aucs, alpha=0.45, linewidth=0.9, label=f'Fold {f_idx+1}')
...
ax.set_xticks(x + 2 * w)
ax.set_xticklabels([f"Fold {r['fold']}" for r in fold_results], fontsize=8)
ax.set_ylim(0, 1.05)
