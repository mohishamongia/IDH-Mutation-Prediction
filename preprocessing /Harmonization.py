...
# ═══════════════════════════════════════════════════════════════════════════
# FULL PIPELINE — wraps all 3 stages per-modality
# ═══════════════════════════════════════════════════════════════════════════
MODALITY_FILES = {
...
# WhiteStripe mode-finding differs by contrast; T1CE behaves like T1
# (WM is relatively hyperintense vs surrounding tissue).
MODALITY_KIND = {"t1": "t1", "t1ce": "t1", "t2": "t2", "flair": "t2"}


def harmonize_subject(subject_dir, nyul_normalizers, n4_kwargs=None):
    """
    Runs N4 -> WhiteStripe -> Nyul for all 4 modalities of one subject.
    ...
    for mod_name, fname in MODALITY_FILES.items():
        ...
        # Stage 3: Nyul (uses the pre-fitted normalizer for this modality)
        vol_nyul = nyul_normalizers[mod_name].transform(vol_ws, mask)
...
def fit_nyul_scales(subject_dirs, save_dir, n4_kwargs=None, n_fit_subjects=None):
    """
    One-time fitting step: runs N4 + WhiteStripe on a training subset,
    then fits a NyulNormalizer per modality and saves the standard scales
    to disk so harmonize_subject() can reuse them for every subject
    (train AND cross-site test) without re-fitting.
